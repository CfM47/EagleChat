...
import (
	...
	messagecache "eaglechat/apps/client/internal/middleware/domain/repositories/message_cache"
)
...
func (m *Middleware) routeIncomingMessages() {
	for rawMsg := range m.p2pConnPool.Receive() {
		...
		if err := json.Unmarshal(rawMsg, &pendingMsg); err != nil {
			log.Printf("middleware: failed to unmarshal incoming message: %v", err)
			continue
		}
...
func (m *Middleware) handleMessageForSelf(pendingMsg middleware_entities.PendingMessage) {
	...
	if err := json.Unmarshal(pendingMsg.Content, &envelope); err != nil {
		log.Printf("middleware: failed to unmarshal secure envelope for own message: %v", err)
		return
	}

	plaintext, senderPubKey, err := simplecrypto.Open(&envelope, &m.sk)
	if err != nil {
		log.Printf("middleware: failed to open secure envelope: %v", err)
		return
	}

	var msg entities.Message
	if err := json.Unmarshal(plaintext, &msg); err != nil {
		log.Printf("middleware: failed to unmarshal inner message: %v", err)
		return
	}

	senderPubKeyBytes, err := senderPubKey.ToBytes()
	if err != nil {
		log.Printf("middleware: failed to serialize sender public key: %v", err)
		return
	}
	msgSenderPubKeyBytes, err := msg.Sender.PublicKey.ToBytes()
	if err != nil {
		log.Printf("middleware: failed to serialize message sender public key: %v", err)
		return
	}
	if !bytes.Equal(senderPubKeyBytes, msgSenderPubKeyBytes) {
		log.Printf("middleware: security alert: sender public key mismatch in message %s", pendingMsg.Target.ID)
		return
	}
...
func (m *Middleware) handleMessageForOther(pendingMsg middleware_entities.PendingMessage) {
	if err := m.messageCache.StoreExpiring(pendingMsg, messagecache.DefaultImmunityPeriod); err != nil {
		log.Printf("middleware: failed to store message for other user %s: %v", pendingMsg.Target.Target, err)
	}
