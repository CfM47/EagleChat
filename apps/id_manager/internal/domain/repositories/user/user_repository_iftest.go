...
func mockUser() *entities.User {
	id := entities.NewUUID()
	return entities.NewUser(id, fmt.Sprintf("user-%s", id), []byte(fmt.Sprintf("pk-%s", id)), time.Now())
}
...
func RunUserRepositoryTests(t *testing.T, repoFactory func(t *testing.T) (UserRepository, func())) {
	...
	t.Run("returns nil IP when IP is expired", func(t *testing.T) {
		...
		ip := net.ParseIP("192.0.2.1")
		user.IP = &ip
		user.LastSeen = entities.NewUser("", "", []byte(""), time.Now()).LastSeen.Add(-entities.IPExpirationDuration - 1*time.Hour)
		require.NoError(t, repo.Save(user))
	...
	t.Run("returns nil IP for expired IPs in FindAll", func(t *testing.T) {
		...
		user1.IP = &ip1
		user1.LastSeen = entities.NewUser("", "", []byte(""), time.Now()).LastSeen.Add(-entities.IPExpirationDuration - 1*time.Hour)
		require.NoError(t, repo.Save(user1))
